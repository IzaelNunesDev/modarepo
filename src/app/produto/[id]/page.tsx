...
export default function ProductPage({ params }: ProductPageProps) {
    ...
    useEffect(() => {
        ...
        Promise.all([getProduct(id), getProducts()])
            .then(([fetchedProduct, allProducts]) => {
                ...
                if (fetchedProduct) {
                    setRelatedProducts(allProducts.filter(p => p.id !== fetchedProduct.id && p.category === fetchedProduct.category).slice(0, 4));
                }
                setLoading(false);
            })
            .catch((err) => {
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push('/')} className="mt-6 px-6 py-2 bg-black text-white rounded-full hover:bg-gray-800 transition">Voltar à Loja</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="relative min-h-screen w-full bg-[#FAFAFA] text-slate-800 selection:bg-pink-100 selection:text-pink-600">
            ...
            <main className="pb-32 lg:pb-12 lg:pt-24 max-w-7xl mx-auto px-0 lg:px-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-12 items-start">
                    ...
                    <div className="relative w-full bg-white lg:rounded-3xl lg:overflow-hidden lg:shadow-xl lg:shadow-slate-100/50 group">
                        ...
                        {/* Thumbnails (Desktop) / Scroll (Mobile) */}
                        <div
                        ...
                        {/* Mobile Title Overlay (Glass effect) */}
                        <div className="absolute bottom-24 left-4 right-4 lg:hidden text-white z-10">
                            ...
                            <h1 className="text-3xl font-black leading-none drop-shadow-md">{product.name}</h1>
                        </div>
                    </div>
                    ...
                    <div className="px-5 pt-6 lg:pt-0 flex flex-col h-full justify-center">
                        ...
                        <div className="hidden lg:block mb-6">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="flex items-center gap-1 text-slate-400 text-sm">
                                    ...
                                    <span className="font-semibold text-slate-700 ml-1">{product.rating.toFixed(1)}</span>
                                    <span>({product.reviewCount} reviews)</span>
                                </div>
                            </div>
                            <h1 className="text-5xl font-black text-slate-900 leading-tight mb-4">{product.name}</h1>
                            <p className="text-lg text-slate-500 leading-relaxed font-light">{product.description}</p>
                        </div>

                        {/* Mobile Description & Rating (Hidden on Desktop) */}
                        ...
                        {/* Selectors Area - Glass Panel Style */}
            ...
            {/* --- Mobile Floating Sticky Bar (Glassmorphism) --- */}
            <div className="fixed bottom-6 left-4 right-4 z-[40] lg:hidden">
                <div className="bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl p-4 flex items-center justify-between gap-4">
                    <div className="flex flex-col">
                        ...
                        <span className="text-xl font-black text-slate-900">R$ {product.price.toFixed(2).replace('.', ',')}</span>
                    </div>
